...
Making the AI verdict *trustworthy* rather than a black box — we force the model to ground every judgment in a verbatim transcript quote. Getting ElevenLabs to conduct a genuinely adaptive, two-way interview (not a scripted bot) via runtime prompt overrides. Supporting 11 languages end-to-end was sneakier than it looked: the candidate's chosen language controlled the UI and the live interview's spoken language, but we initially missed that the *generation* step (OpenAI writing the rubric, aptitude questions, and interview questions) never received that language — so a role set to Malay still generated English questions. Fixing it meant pushing the language choice all the way through the prompt that drafts the assessment, not just the prompt that runs the interview. And making a glowing 3D orb read well on a clean, professional light theme rather than a "tech demo" dark UI.
